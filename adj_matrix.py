...
#label_dict = convert_to_300('../')

tag_dict = {}
for i in range(1):
	tag_dict.update(dict(np.load(os.path.join('../', 'metadata_{}.npy'.format(i))).item()))
...
col_lst = ['solo', '1girl', '1boy', '2girls', 'multiple_girls', 'multiple_boys']
row_lst = ['long_hair', 'short_hair', 'twintails', 'bangs', 'very_long_hair', 'ponytail', 'hair_ribbon',
'hair_ornament', 'hair_bow', 'braid', 'hairband', 'ahoge', 'solo', '1girl', '1boy', '2girls', 'multiple_girls', 'multiple_boys']
adj_mat = [[0 for j in range(len(col_lst))] for i in range(len(row_lst))]

name_lst = []
feat_lst = []
i = 0
num_total_file = len(label_dict)
for tag_lst in label_dict:
	for row in range(len(row_lst)):
		for col in range(len(col_lst)):
			if (col_lst[col] in label_dict[tag_lst]) and (row_lst[row] in label_dict[tag_lst]):
				adj_mat[row][col] += 1
				name_lst.append(col_lst[col])
				feat_lst.append(row_lst[row])

	i += 1
	if i % 1000 == 0:
		print('{0:3f}% Done'.format(100*(i/num_total_file)))


f = open('./name_lst.txt', 'w')
f.write(str(name_lst))
f.close() 

f = open('./feat_lst.txt', 'w')
f.write(str(feat_lst))
f.close() 
np.save('./name_lst.npy',name_lst)
np.save('./feat_lst.npy',feat_lst)

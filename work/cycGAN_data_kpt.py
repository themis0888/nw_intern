...
--save_path=/shared/data/meta/celeb/cycGAN_middle_input/ \
...
parser.add_argument('--save_path', type=str, dest='save_path', default='/shared/data/meta/anime/cycGAN_middle_input/')
...
def cycGAN_data(path, kpt):
	...
	if not isinstance(img, np.ndarray):
		return path
	h, w, c = img.shape
	kpt_layer = np.zeros([h,w,1]) + 127.5
	for i in range(68):
...
for i, image_path in enumerate(image_path_list):
	...
	if i % 500 == 0:
		print('{0:02f}% done'.format(100*i/num_tot))
